...
int main() {
  ...
  msg.msg_name = name;  // source address
  ...
  printf("name:\nlen:%u\n", msg.msg_namelen);
  char ip_toshow[1000] = {0};
  inet_ntop(AF_INET, msg.msg_name, ip_toshow, 1000);
  printf("value: %u\n", *(unsigned int*)msg.msg_name);
  printf("ipv4 : %s\n", ip_toshow);
  for (unsigned int i = 0; i < msg.msg_namelen; ++i) {
    printf("%02x ", ((unsigned char*)msg.msg_name)[i]);
  }
  printf("\n\n");
  ...
  while (cmhdr) {
    printf("cmhdr: %p, level: %d, len: %ld\n", cmhdr, cmhdr->cmsg_level,
           cmhdr->cmsg_len);
    // if (cmhdr->cmsg_level == IPPROTO_IP) {
    recved = CMSG_DATA(cmhdr);
    for (int l = 0; l < size / 16 + 1; ++l) {
      for (int i = 0; i < 16 && l * 16 + i < size; ++i)
        printf("%02x ", recved[l * 16 + i]);
      printf("\n");
